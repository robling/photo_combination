...
namespace photo_combination
{
    class Canny
    {
        ...
        public Bitmap DisplayImage()
        {
            ...
            byte[] imagePointer1 = new byte[image.Width * image.Height * 4];
            ...
            for (i = 0; i < image.Height; i++)
            {
                for (j = 0; j < image.Width; j++)
                {
                    ...
                    imagePointer1[j + i * image.Width + 0] = (byte)GreyImage[j, i];
                    imagePointer1[j + i * image.Width + 1] = (byte)GreyImage[j, i];
                    imagePointer1[j + i * image.Width + 2] = (byte)GreyImage[j, i];
                    imagePointer1[j + i * image.Width + 3] = (byte)255;
                    //4 bytes per pixel
                    //imagePointer1 += 4;
                }
            }
        ...
        public Bitmap DisplayImage(float[,] GreyImage)
        {
            ...
            for (i = 0; i < image.Height; i++)
            {
                ...
                {
                    // write the logic implementation here
                    imagePointer1[(j + i * image.Width) * 4 + 0] = (byte)GreyImage[j, i];
                    imagePointer1[(j + i * image.Width) * 4 + 1] = (byte)GreyImage[j, i];
                    imagePointer1[(j + i * image.Width) * 4 + 2] = (byte)GreyImage[j, i];
                    imagePointer1[(j + i * image.Width) * 4 + 3] = (byte)255;
                    //4 bytes per pixel
                    //imagePointer1 += 4;
                }
            }
            image = photo_combination.ImageDataConverter.ToBitmap(imagePointer1, image.Width, image.Height);
            return image;// col;
        }      // Display Grey Imag
        ...
        private void ReadImage()
        {
            ...
            byte[] imagePointer1 = photo_combination.ImageDataConverter.ToByteArray(image);

            for (i = 0; i < image.Height; i++)
            {
                for (j = 0; j < image.Width; j++)
                {
                    GreyImage[j, i] = (int)((imagePointer1[(j + i * image.Width) * 4 + 0] + imagePointer1[(j + i * image.Width) * 4 + 1] + imagePointer1[(j + i * image.Width) * 4 + 2]) / 3.0);
                }
            }
        }
